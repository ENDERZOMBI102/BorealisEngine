...
impl Upkf {
	...
	pub fn load( path: &Path, check_content: bool ) -> Result<Self, UpkfError> {
		...
		for _index in 0 .. header.entry_count {
			let entry_header = EntryHeader::load( &mut file )?;
			chksum += entry_header.crc as u128;
			let entry = Entry::load( &mut file, &entry_header )?;
			upkf.entries.push( Element::load( entry_header, entry, check_content )? );
		}
	...
	fn write<'a>( &self, file: &'a mut dyn Write ) -> Result<u32, UpkfError> {
		...
		match self.compression {
			...
			CompressionType::GZIP => {
				let mut encoder = libflate::gzip::Encoder::new( &mut bytes )?;
...
impl FileHeader {
	...
	fn load<'a>( file: &'a mut dyn Read ) -> Result<Self, UpkfError> {
		...
		let recompressed = file.read_u8()? != 0;
		let origin_size = file.read_u16::<LittleEndian>()?;
		let mut buf = vec![ 0 as u8; origin_size as usize ];
		file.read_exact( &mut buf )?;
		let origin = String::from_utf8( buf )?;
		let chksum = file.read_u128::<LittleEndian>()?;
